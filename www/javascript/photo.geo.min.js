...
function _photo_geo_context_update_onsubmit(){var a=$("#edit_geo").attr("geo:context"),b=$("#new_geocontext");b=b.val();if(b==a)alert("Hey! There's nothing to update!");else{$.ajax({url:"/api/rest/",type:"POST",data:{method:"flickr.photos.geo.setContext",photo_id:photo_id,context:b},success:_photo_geo_set_context_onsuccess});$("#photo_geo_context").hide();_photo_geo_status_show("Poking the Flickr API...");return false}}
...
function _photo_geo_corrections_for_placetype(a){$.ajax({url:"/api/rest/",type:"GET",data:{method:"flickr.photos.geo.possibleCorrections",photo_id:photo_id,place_type:a},success:_photo_geo_possible_corrections_onsuccess});_photo_geo_status_show("Fetching alternate place names...")}
...
function _photo_geo_corrections_update_onsubmit(){var a=$("#new_woeid");a=a.val();if(a==""){alert("Hey! You didn't choose anything.");return false}if(a==-1){a=$("#new_woeid option:last-child").attr("geo:placetype");_photo_geo_corrections_for_placetype(a);return false}$.ajax({url:"/api/rest/",type:"POST",data:{method:"flickr.photos.geo.correctLocation",photo_id:photo_id,woeid:a},success:_photo_geo_correct_location_onsuccess});$("#photo_geo_corrections").hide();_photo_geo_status_show("Poking the Flickr API...");
return false}
function _photo_geo_correct_location_onsuccess(a){_photo_geo_status_hide();if(a.stat!="ok")_photo_geo_flickr_error(a);else{var b=a.woeid,d=a.place.name;$("#edit_geo").attr("geo:woeid",a.woeid);placename=b==0?"a place with no name":places_is_enabled?'<a href="'+(places_url+b+"/")+'">'+d+"</a>":d;$("#geo_placename").html(placename);$("#photo_geo_status").html("Okay! The place name for your photo has been updated. It is now "+d);$("#photo_geo_status").show();setTimeout(function(){_photo_geo_status_hide();$("#photo_geo_corrections").html(_photo_geo_corrections_generate_html);
