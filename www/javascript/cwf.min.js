...
function cwf_init_shortcuts(){var a=function(){cwf_show_previous_photo("overflow")},b=function(){cwf_show_next_photo("overflow")},c=function(){cwf_show_photo(0)},e=function(){cwf_show_photo(photos.length-1)};$(document).keydown(function(d){if(d.keyCode==37)a();else if(d.keyCode==38)c();else if(d.keyCode==39)b();else d.keyCode==40&&e()});$(document).touchwipe({wipeLeft:b,wipeRight:a,wipeUp:c,wipeDown:e,min_move_x:20,min_move_y:20,preventDefaultEvents:true})}
...
function _cwf_check_photos_callback(a){a=JSON.parse(a);var b=a.photos;a=b.length;console.log("pre: p:"+a+" u:"+count_updates);if(a){b.reverse();for(var c=[],e=0;e<a;e++){var d=b[e];c.push(d.display_url);c.push(d.thumb_url);photos.unshift([d.photo_id,d.title,d.owner,d.ownername,d.faved_by_nsid,d.faved_by,d.thumb_url,d.display_url])}$({}).imageLoader({images:c,async:true});idx+=a;count_updates+=a;b='<a href="#" onclick="cwf_show_photo(0);return false;">';console.log("post: p:"+a+" u:"+count_updates);
b+=count_updates>1?"there are "+count_updates+" new faves":"there are new faves";b+="</a>";$("#cwf_updates").html(b);$("#cwf_photo_idx").html(idx+1);$("#cwf_count_photos").html(photos.length)}}function cwf_next_photo(a){if(a)return idx<photos.length-1?idx+1:0;return idx<photos.length-1?idx+1:-1}function cwf_previous_photo(a){if(a)return idx>0?idx-1:count_photos-1;return idx>0?idx-1:-1}function cwf_show_previous_photo(a){a=cwf_previous_photo(a);cwf_show_photo(a)}
function cwf_show_next_photo(a){a=cwf_next_photo(a);cwf_show_photo(a)}
...
msg+=" / "+photo_favorites_generate_html(b,e);msg+='<div id="cwf_updates"></div>';msg+="</div>";$("#cwf_about").html("");$.backstretch(i);$("#cwf_about").html(msg)};
