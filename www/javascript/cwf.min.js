var photos=[],thumbs=[],images=[],count_photos=0,count_updates=0,idx=0;
function cwf_init(a){photos=a;count_photos=photos.length;cwf_init_layout();cwf_init_shortcuts();for(a=0;a<count_photos;a++){thumbs.push(photos[a][6]);images.push(photos[a][7])}if(a=location.hash)if((a=parseInt(a.substring(1,a.length)))&&a<=count_photos)idx=a-1;$.backstretch(thumbs[idx]);$({}).imageLoader({images:thumbs,async:true});$({}).imageLoader({images:[images.shift()],async:false,complete:function(){cwf_show_photo(idx)}});a=parseInt(new Date/1E3);cwf_schedule_check_photos(a)}
...
msg+=" / "+photo_favorites_generate_html(b,d);msg+='<div id="cwf_updates"></div>';msg+="</div>";$("#cwf_about").html("");$.backstretch(i);$("#cwf_about").html(msg)}function cwf_fave_photo(a){$.ajax({url:"/api",type:"POST",data:{method:"flickr.favorites.add",photo_id:a},success:_cwf_fave_photo_onsuccess})}function _cwf_fave_photo_onsuccess(a){a=$("#cwf_fave_"+a.photo_id);a=$(a);a.html("\u2665");a.attr("onclick","alert('hi');")};
